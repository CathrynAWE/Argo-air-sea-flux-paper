...
S55_float_data.TEMP = temp;
S55_float_data.pH = pH;
S55_float_data.oxy = oxy;
S55_float_data.pres = pres;
S55_float_data.psal = S;
...
for n = 1:fs(2)
    ...
    S55_float_data.pH_LIR_Deep(:,n) = pH(:,n)-adj_LIR_Deep.offset(idx_L_D)-adj_LIR_Deep.drift(idx_L_D)*(JULD(n)-JULD(adj_LIR_Deep.Cycle_start(idx_L_D)))/365;
    S55_float_data.pH_LIR_Shallow(:,n) = pH(:,n)-adj_LIR_Shallow.offset(idx_L_S)-adj_LIR_Shallow.drift(idx_L_S)*(JULD(n)-JULD(adj_LIR_Shallow.Cycle_start(idx_L_S)))/365;
    S55_float_data.pH_Williams_Deep(:,n) = pH(:,n)-adj_Williams_Deep.offset(idx_W_D)-adj_Williams_Deep.drift(idx_W_D)*(JULD(n)-JULD(adj_Williams_Deep.Cycle_start(idx_W_D)))/365;
...
%%% reference depth
for i= 1:size(S55_float_data.pH,2)
    pres_ref_id_D = S55_float_data.pres(:,i)>=(ref_depth_D-ref_depth_t_D) & S55_float_data.pres(:,i)<=(ref_depth_D+ref_depth_t_D); 
    pH_ref.pres_D(i) = mean(S55_float_data.pres(pres_ref_id_D,i),'omitnan');
    %%% find the pH at the reference depths
    pH_ref.pH_D(i) = mean(S55_float_data.pH_LIR_Deep(pres_ref_id_D,i),'omitnan');
    pH_ref.temp_D(i) = mean(S55_float_data.TEMP(pres_ref_id_D,i),'omitnan');
    pH_ref.psal_D(i) = mean(S55_float_data.psal(pres_ref_id_D,i),'omitnan');
...
%%% reference depth
for i= 1:size(S55_float_data.pH,2)
    pres_ref_id_S = S55_float_data.pres(:,i)>=(ref_depth_S-ref_depth_t_S) & S55_float_data.pres(:,i)<=(ref_depth_S+ref_depth_t_S); 
    pH_ref.pres_S(i) = mean(S55_float_data.pres(pres_ref_id_S,i),'omitnan');
    %%% find the pH at the reference depths
    pH_ref.pH_S(i) = mean(S55_float_data.pH_LIR_Deep(pres_ref_id_S,i),'omitnan');
    pH_ref.temp_S(i) = mean(S55_float_data.TEMP(pres_ref_id_S,i),'omitnan');
    pH_ref.psal_S(i) = mean(S55_float_data.psal(pres_ref_id_S,i),'omitnan');
    %%% convert the pH at the reference depths to what it would be at 25C
...
%for i=15   
    ...
    S55_float_data.pH_LD_corr(:,i) = S55_float_data.pH_LIR_Deep(:,i)+pH_ref.pH_corr_D(idx);
    S55_float_data.pH_WD_corr(:,i) = S55_float_data.pH_Williams_Deep(:,i)+pH_ref.pH_corr_D(idx);
end

for i = 1:size(S55_float_data.pH,2)
    ind = knnsearch(pH_ref.profile_no_S(:),i);
    idx = pH_ref.profile_no_S(ind);
    S55_float_data.pH_LS_corr(:,i) = S55_float_data.pH_LIR_Shallow(:,i)+pH_ref.pH_corr_S(idx);
end
...
%for i =1:3
    ...
    p_L_D = accumarray(c,S55_float_data.pH_LIR_Deep(msk,i),[],@(x)mean(x,'omitnan'));
    pH_L_D_20(end+1,1) = p_L_D(2);
    p_L_S = accumarray(c,S55_float_data.pH_LIR_Shallow(msk,i),[],@(x)mean(x,'omitnan'));
    pH_L_S_20(end+1,1) = p_L_S(2);
    p_W_D = accumarray(c,S55_float_data.pH_Williams_Deep(msk,i),[],@(x)mean(x,'omitnan'));
    pH_W_D_20(end+1,1) = p_W_D(2);
    p_L_D_corr = accumarray(c,S55_float_data.pH_LD_corr(msk,i),[],@(x)mean(x,'omitnan'));
    pH_L_D_corr_20(end+1,1) = p_L_D_corr(2); 
    p_L_S_corr = accumarray(c,S55_float_data.pH_LS_corr(msk,i),[],@(x)mean(x,'omitnan'));
    pH_L_S_corr_20(end+1,1) = p_L_S_corr(2); 
    p_W_D_corr = accumarray(c,S55_float_data.pH_WD_corr(msk,i),[],@(x)mean(x,'omitnan'));
    ...
    temp_20(end+1,1) = t(2);
    S55_float_data.Alk_pres(1:size(oxy(msk,i),1),end+1) = pres(msk,i);
...
S55_float_data.Alk_LIAR = Alk_LIAR;
S55_float_data.Alk_ES = Alk_ES;
...
for i=1:length(Alk_LIAR_20)
    ...
    S55_float_data.pCO2_L_D(i) = DATA_L_D(22); % uatm
    ...
    S55_float_data.pCO2_L_D_20m(i) = DATA_L_D_20(22); % uatm
    ...
    S55_float_data.pCO2_L_S(i) = DATA_L_S(22); % uatm
    ...
    S55_float_data.pCO2_W_D(i) = DATA_W_D(22); % uatm 
    ...
    S55_float_data.pCO2_L_D_corr(i) = DATA_L_D_corr(22); % uatm
    ...
    S55_float_data.pCO2_L_S_corr(i) = DATA_L_S_corr(22); % uatm
    ...
    S55_float_data.pCO2_W_D_corr(i) = DATA_W_D_corr(22); % uatm
    ...
    S55_float_data.pCO2_L_D_ES(i) = DATA_L_D_ES(22); % uatm
    ...
    S55_float_data.pCO2_L_S_ES(i) = DATA_L_S_ES(22); % uatm
    ...
    S55_float_data.pCO2_W_D_ES(i) = DATA_W_D_ES(22); % uatm 
    
    S55_float_data.lat(i) = lat(i);
    S55_float_data.lon(i) = lon(i);
    S55_float_data.time(i) = time_float(i);
    S55_float_data.TEMP_20(i) = temp_20(i);
    S55_float_data.PSAL_20(i) = S_20(i);
    S55_float_data.pH_L_D_20(i) = pH_L_D_20(i);
    S55_float_data.pH_L_S_20(i) = pH_L_S_20(i);
    S55_float_data.pH_W_D_20(i) = pH_W_D_20(i);
    S55_float_data.pH_L_D_20_corr(i) = pH_L_D_corr_20(i);
    S55_float_data.pH_L_S_20_corr(i) = pH_L_S_corr_20(i);
    S55_float_data.pH_W_D_20_corr(i) = pH_W_D_corr_20(i);
        
    S55_float_data.Alk_LIAR_20(i) = Alk_LIAR_20(i);
    S55_float_data.Alk_ES_20(i) = Alk_ES_20(i);
    S55_float_data.pres_20(i) = pres_20(i);
end

S55_float_data.pCO2_L_D(S55_float_data.pCO2_L_D== -999) = NaN;
S55_float_data.pCO2_L_D_20m(S55_float_data.pCO2_L_D_20m== -999) = NaN;
S55_float_data.pCO2_L_S(S55_float_data.pCO2_L_S == -999) = NaN;
S55_float_data.pCO2_W_D(S55_float_data.pCO2_W_D== -999) = NaN;
S55_float_data.pCO2_L_D_corr(S55_float_data.pCO2_L_D_corr== -999) = NaN;
S55_float_data.pCO2_L_S_corr(S55_float_data.pCO2_L_S_corr== -999) = NaN;
S55_float_data.pCO2_W_D_corr(S55_float_data.pCO2_W_D_corr == -999) = NaN;
S55_float_data.pCO2_L_D_ES(S55_float_data.pCO2_L_D_ES== -999) = NaN;
S55_float_data.pCO2_L_S_ES(S55_float_data.pCO2_L_S_ES== -999) = NaN;
S55_float_data.pCO2_W_D_ES(S55_float_data.pCO2_W_D_ES== -999) = NaN;
...
%for i = 156   
    ...
    DpCO2_L_D = S55_float_data.pCO2_L_D(i)-pCO2_uatm;
    DpCO2_L_D_20m = S55_float_data.pCO2_L_D_20m(i)-pCO2_uatm;
    DpCO2_L_S = S55_float_data.pCO2_L_S(i)-pCO2_uatm;
    DpCO2_W_D = S55_float_data.pCO2_W_D(i)-pCO2_uatm;
    DpCO2_L_D_corr = S55_float_data.pCO2_L_D_corr(i)-pCO2_uatm;
    DpCO2_L_S_corr = S55_float_data.pCO2_L_S_corr(i)-pCO2_uatm;
    DpCO2_W_D_corr = S55_float_data.pCO2_W_D_corr(i)-pCO2_uatm;
...
%     [F_CO2_float_W_D]=FCO2_CWE(DpCO2_W_D,t_2_f,S_20(i),wsp_f);
    DpCO2_L_D_ES = S55_float_data.pCO2_L_D_ES(i)-pCO2_uatm;
    DpCO2_L_S_ES = S55_float_data.pCO2_L_S_ES(i)-pCO2_uatm;
    DpCO2_W_D_ES = S55_float_data.pCO2_W_D_ES(i)-pCO2_uatm;
%     [F_CO2_float_L_D_ES_airtemp]=FCO2_CWE(DpCO2_L_D_ES,t_2_f,S_20(i),wsp_f);
    ...
    S55_float_data.flux_L_D(i) = F_CO2_float_L_D;
    S55_float_data.flux_L_D_20m(i) = F_CO2_float_L_D_20m;
    S55_float_data.flux_L_S(i) = F_CO2_float_L_S;
    S55_float_data.flux_W_D(i) = F_CO2_float_W_D;
    S55_float_data.flux_L_D_corr(i) = F_CO2_float_L_D_corr;
    S55_float_data.flux_L_S_corr(i) = F_CO2_float_L_S_corr;
    S55_float_data.flux_W_D_corr(i) = F_CO2_float_W_D_corr;
    

    S55_float_data.flux_L_D_ES(i) = F_CO2_float_L_D_ES;
%     SOTS_float_data.flux_L_D_ES_airtemp(i) = F_CO2_float_L_D_ES_airtemp;
    S55_float_data.flux_L_S_ES(i) = F_CO2_float_L_S_ES;
    S55_float_data.flux_W_D_ES(i) = F_CO2_float_W_D_ES;
    
    S55_float_data.pCO2_uatm(i) = pCO2_uatm;
    S55_float_data.CG_xCO2_uatm(i) = CGdata_interp.ppm_spl(idx_C);
    S55_float_data.wsp(i) = wsp_f;
    S55_float_data.t_2(i) = t_2_f;
    S55_float_data.msp(i) = msp_f;
end

%%%% now I just want 2021, because it is a full year
S55_float_data.Month = month(S55_float_data.time);
S55_float_data.year = year(S55_float_data.time);

S55_float_data.data_comp_2021 = table(S55_float_data.Month(S55_float_data.year==2021)',...
    S55_float_data.time(S55_float_data.year==2021)',...
    S55_float_data.pH_L_D_20(S55_float_data.year==2021)',S55_float_data.pH_L_S_20(S55_float_data.year==2021)',...
    S55_float_data.pH_W_D_20(S55_float_data.year==2021)',S55_float_data.pH_L_D_20_corr(S55_float_data.year==2021)',...
    S55_float_data.pH_L_S_20_corr(S55_float_data.year==2021)',S55_float_data.pH_W_D_20_corr(S55_float_data.year==2021)',...
    S55_float_data.Alk_LIAR_20(S55_float_data.year==2021)',S55_float_data.Alk_ES_20(S55_float_data.year==2021)',...
    S55_float_data.flux_L_D(S55_float_data.year==2021)', S55_float_data.flux_L_S(S55_float_data.year==2021)',...
    S55_float_data.flux_W_D(S55_float_data.year==2021)',S55_float_data.flux_L_D_corr(S55_float_data.year==2021)',...
    S55_float_data.flux_L_S_corr(S55_float_data.year==2021)',S55_float_data.flux_W_D_corr(S55_float_data.year==2021)',...
    S55_float_data.flux_L_D_ES(S55_float_data.year==2021)', S55_float_data.flux_L_S_ES(S55_float_data.year==2021)',...
    S55_float_data.flux_W_D_ES(S55_float_data.year==2021)', 'VariableNames',{'Month','Time','pH_LD_20','pH_LS_20',...
...
S55_float_data.pH_LD_20_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.pH_LD_20,[],@(x)mean(x,'omitnan'));
S55_float_data.pH_LS_20_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.pH_LS_20,[],@(x)mean(x,'omitnan'));
S55_float_data.pH_WD_20_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.pH_WD_20,[],@(x)mean(x,'omitnan'));
S55_float_data.pH_LD_20_corr_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.pH_LD_20_corr,[],@(x)mean(x,'omitnan'));
S55_float_data.pH_LS_20_corr_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.pH_LS_20_corr,[],@(x)mean(x,'omitnan'));
S55_float_data.pH_WD_20_corr_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.pH_WD_20_corr,[],@(x)mean(x,'omitnan'));

S55_float_data.Alk_LIAR_20_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.Alk_LIAR_20,[],@(x)mean(x,'omitnan'));
S55_float_data.Alk_ES_20_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.Alk_ES_20,[],@(x)mean(x,'omitnan'));
S55_float_data.flux_LD_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_LD,[],@(x)mean(x,'omitnan'));
S55_float_data.flux_LS_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_LS,[],@(x)mean(x,'omitnan'));
S55_float_data.flux_WD_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_WD,[],@(x)mean(x,'omitnan'));
S55_float_data.flux_LD_corr_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_LD_corr,[],@(x)mean(x,'omitnan'));
S55_float_data.flux_LS_corr_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_LS_corr,[],@(x)mean(x,'omitnan'));
S55_float_data.flux_WD_corr_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_WD_corr,[],@(x)mean(x,'omitnan'));

S55_float_data.flux_LD_ES_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_LD_ES,[],@(x)mean(x,'omitnan'));
S55_float_data.flux_LS_ES_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_LS_ES,[],@(x)mean(x,'omitnan'));
S55_float_data.flux_WD_ES_mo_ave = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_WD_ES,[],@(x)mean(x,'omitnan'));

S55_float_data.flux_LD_corr_mo_SD = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_LD_corr,[],@(x)std(x,'omitnan'));
S55_float_data.flux_LS_corr_mo_SD = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_LS_corr,[],@(x)std(x,'omitnan'));
S55_float_data.flux_WD_corr_mo_SD = accumarray(S55_float_data.data_comp_2021.Month,S55_float_data.data_comp_2021.flux_WD_corr,[],@(x)std(x,'omitnan'));


S55_float_data.mo_ave_month = unique(S55_float_data.data_comp_2021.Month);
...
if float_ID == '5906623'
    ...
    %for i = 156   
        ...
        S55_float_data.mooring_lat(i) = mooring_data.xCO2_lat(idx_lat);
        S55_float_data.mooring_lon(i) = mooring_data.xCO2_lon(idx_lon);
